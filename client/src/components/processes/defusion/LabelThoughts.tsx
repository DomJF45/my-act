...
import { Form, Card, Button, FormControlProps } from 'react-bootstrap';
...
const LabelThoughts = () => {
  ...
  const [form, newForm]:[iForm[], (form: iForm[]) => void] = useState([
    {thought: 'n/a', label: 'n/a'}
  ]);

  const handleFormChange = (index: number, e: React.ChangeEvent<any>) => {
    let data = [...form];
    data[index][e.currentTarget.name as keyof iForm] = e.currentTarget.value;
    newForm(data);
  }
