...
const Modal = (props: Props) => {
  ...
  return (
    <div className={props.modalShow ? 'background' : 'background-hidden'}>
      <div className={props.modalShow ? 'my-modal-container' : 'my-modal-container-hidden'}>
        <div className='my-modal-close' onClick={() => props.setModalShow(false)}>
          <RiCloseLine size={30}  />
        </div>
          ...
          <div className="my-modal-content">
            <div className="my-modal-title-form">
              ...
              <Form className="entry-form" id="entry-form">
                <h4>Title</h4>
                <Form.Control placeholder='title' value={title} onChange={(e: React.ChangeEvent<HTMLInputElement>): void => setTitle(e.target.value)} />
                <h4 style={{marginTop: '1rem'}}>Body</h4>
                <Form.Control placeholder='body' as='textarea' rows={8} value={body} onChange={(e: React.ChangeEvent<HTMLInputElement>): void => setBody(e.target.value)} />
              </Form>
            </div>
          </div>
        </div>
